...
### 自動車関係は書かない。書くなら審査を通す

**車・CAN・HV システムに関する内容を、勝手にサイトへ載せないこと**
(2026-08-18 本人指示)。seeds / log / garden / works のどの層でも同じ。

- **下書きまでは作ってよい。** ただし**本人に見せて承認を得るまでコミットしない**
- 「技術的に面白いから」は理由にならない。**判断するのは本人**
- 対象は `~/obdlink-lx-eval` 由来のもの全般(CAN ログ、ID の同定、車両の挙動、
  HV バスへの介入)と、そこから派生した考察

これは機密の話ではなく**公開の可否を本人が握る**という話。
迷ったら書かない方に倒す。
