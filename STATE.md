---
schema_version: 2
last_updated: 2026-09-17T12:31+09:00
current_focus: "録画が受信からエンコード・退避まで一本でつながった（録画14本/エンコ済み10本）。いまはアトリエのローカルLLMに回す仕事を作っている——ログ蒸留(545KB→0.4%)は完成、会話ログ150本の索引づくりと記事リンクの候補出しは道具ができて安い枠のキューに入った"
projects:
  - slug: foundation
    status: active
  ...
  - slug: q3pe-recorder
    status: active
    summary: "遊んでいた PX-Q3PE を kernel 7.0 で生き返らせた。B-CAS 復号まで動き、録画14本・エンコード済み10本。録画→CM検出(自宅)→安い枠でアトリエの GPU エンコ→Drive まで一本につながった"
...
1. **録画が一本の流れになった** — 受信（地上波34dB/衛星14dB）から B-CAS 復号、
   ...
   録画14本・エンコード済み10本。**1枠に2本入ることを実測**（45分番組で8〜15分）。
